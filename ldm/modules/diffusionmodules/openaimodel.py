...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.emb_layers = nn.Sequential(     #原版
            nn.SiLU(),
            linear(
                emb_channels,
...
class AttentionBlock(nn.Module):
    def __init__(
        ...
        if emb_channels is not None:
            ...
            self.cond_proj = linear(emb_channels, channels)   # 直接映射到 C 维
        else:
            self.cond_act = None
            self.cond_proj = None
    ...
    def _forward(self, x, emb=None):
        b, c, *spatial = x.shape
        x_in = x.reshape(b, c, -1)   # [B, C, T]

        x_norm = self.norm(x_in)

        if emb is not None and self.cond_proj is not None:
            cond = self.cond_act(emb)               # [B, emb_channels]
            cond = self.cond_proj(cond).type(x_norm.dtype)  # [B, C]
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.predict_codebook_ids:
            ...
            print("id_predictor:", self.id_predictor)
