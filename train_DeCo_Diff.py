...
def main(args):
    ...
    diffusion = create_diffusion(timestep_respacing="ddim10", predict_deviation=False, predict_xstart=False, sigma_small=False, diffusion_steps=10)
    ...
    # opt = torch.optim.AdamW(list(model.parameters()) + list(vae.parameters()), lr=args.lr, weight_decay=1e-4)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for ii, (x, _, y) in enumerate(loader):
            ...
            # print('args.actual_image_size:', args.actual_image_size)
            ...
            if train_steps < 3 and rank == 0:
                print("x mean/std:", x.mean().item(), x.std().item())
                print("mask mean:", mask.mean().item())
                print("loss_dict:", {k: v.mean().item() for k,v in loss_dict.items()})
            ...
            running_mse += loss_dict["loss"].mean().item()
            ...
            if train_steps % args.log_every == 0:
                ...
                avg_mse  = torch.tensor(running_mse  / log_steps, device=device)   # mse-loss

                # dist.all_reduce(avg_loss, op=dist.ReduceOp.SUM)
                dist.all_reduce(avg_mse,  op=dist.ReduceOp.SUM)

                # avg_loss = avg_loss.item() / dist.get_world_size()   # v-loss
                avg_mse  = avg_mse.item()  / dist.get_world_size()   # mse-loss

                logger.info(
                    f"(category={args.object_category} step={train_steps:07d}) "
                    # f"V-Loss: {avg_loss:.4f}, "
                    f"MSE Loss: {avg_mse:.4f}, "
                    f"Train Steps/Sec: {steps_per_sec:.2f}"
                )

                # Reset monitoring variables:
                running_mse  = 0    # mse-loss
                running_mt   = 0
                log_steps    = 0
                start_time   = time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--center-crop", type=lambda v: True if v.lower() in ('yes','true','t','y','1') else False, default=False)
    ...
    parser.add_argument("--med-data-dir", type=str, default='./data')
